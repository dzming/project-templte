...
const key = CryptoJS.enc.Base64.parse(env.VUE_APP_AESKEY); // 十六位十六进制数作为密钥
const iv = CryptoJS.enc.Base64.parse(env.VUE_APP_AESIV); // 十六位十六进制数作为密钥偏移量

const signKey = env.VUE_APP_SIGNKEY;
const signVal = env.VUE_APP_SIGNVALUE;
